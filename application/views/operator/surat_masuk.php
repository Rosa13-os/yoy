							...
							<table cellspacing="0" class="display nowrap table table-hover table-striped table-bordered tableku" width="100%">
								...
								<tbody id="isi">
									...
									<tr>
										...
										<td>
											<td><img src="<?=base_url('upload/masuk/'.$key->foto)?>" style="height:50px">
						<td>
							<a href ="<?=site_url('operator/detailsuratmasuk/'.$key->id_smasuk)?>" class="btn btn-primary fa fa-eye" title="detail">
								<!-- <i class="fa fa-eye"></i> Detail -->
							</a>
							<a data-toggle="modal" data-target="#update<?=$key->id_smasuk?>" href="javascript:void()" class="btn btn-primary fa fa-pencil" title="edit">
								<!-- <i class="fa fa-pencil"></i> Edit -->
							</a>
							<a href ="<?=site_url('operator/delmasuk/'.$key->id_smasuk)?>" onclick="return confirm('Apakah Anda Ingin Menghapus Data <?=$key->no_surat;?> ?');"class="btn btn-primary fa fa-trash">
...
<div class="modal fade text-xs-left" id="update<?php echo $e->id_smasuk ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			...
			<form method="post" action="<?php echo base_url('operator/updatemasuk/'.$e->id_smasuk) ?>" enctype="multipart/form-data">
...
<script>
	...
	function carinosurat2(id) {
		if ($("#no_surat"+id.toString()).val()!=='') {
			$.ajax({
				type: "GET",
				url: "<?php echo base_url('operator/carinosurat/') ?>"+$("#no_surat"+id.toString()).val(),
				success: function (data) {
					var dataa = data;
					if (dataa==1) {
